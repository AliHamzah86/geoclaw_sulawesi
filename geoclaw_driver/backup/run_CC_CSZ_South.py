...
def iter_fun(self):
 ...
 grid-size  |           coarse          |           fine            | c | f |
    ...
    def load_fgmax_from_txt(fgfile):
        fg = fgmax_tools.FGmaxGrid()
        with open(os.path.join(driver_home, fgfile)) as f:
        ...
        fg.tstart_max = float(tokens[0][0])
        fg.tend_max   = float(tokens[1][0])
        fg.dt_check   = float(tokens[2][0])
        fg.min_level_check = int(float(tokens[3][0]))   # ✅ fix here
        fg.arrival_tol = float(tokens[4][0])
        fg.point_style = int(float(tokens[5][0]))       # also safe cast

        if fg.point_style == 0:
            fg.xy_fname = os.path.join(driver_home, tokens[6][0])
        elif fg.point_style == 1:
            fg.npts = int(float(tokens[6][0]))
            fg.xy_fname = os.path.join(driver_home, tokens[7][0])
        elif fg.point_style == 2:
            nx, ny = map(int, map(float, tokens[6]))
            fg.nx, fg.ny = nx, ny
            fg.x1, fg.y1 = map(float, tokens[7])
            fg.x2, fg.y2 = map(float, tokens[8])
        return fg
    ...
    if run_id == N:
        ...
        if ((run_id >= 0) and (run_id < 4*M)) or (run_id == 8*M):
            ...
            # for test <<<<<<<<<<<<<<<<<<<<<<< 
            fgmax_grids = []
            for fgfile in ['fgmax1_coarse.txt', 'fgmax2_coarse.txt', 'fgmax3_coarse.txt']:
                fgmax_grids.append(load_fgmax_from_txt(fgfile))

            self._rundata.fgmax_data.fgmax_grids = fgmax_grids
            ...
            # for test <<<<<<<<<<<<<<<<<<<<<<<
            fgmax_grids = []
            for fgfile in ['fgmax1_fine.txt', 'fgmax2_fine.txt', 'fgmax3_fine.txt']:
                fgmax_grids.append(load_fgmax_from_txt(fgfile))
