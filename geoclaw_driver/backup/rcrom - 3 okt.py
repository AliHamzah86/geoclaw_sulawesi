...
class Drom(object):
    ...
    def evaluate_hdm(self, save_drom=False, return_grb=False):
        ...
        if (self._check_run() and self._continue):
            ...
            # Ensure a proper Makefile exists in rundir
            makefile_target = os.path.join(rundir, "Makefile")
            if not os.path.exists(makefile_target):
                makefile_src = os.path.join(self.path_common, "Makefile")
                if not os.path.exists(makefile_src):
                    # fallback to the system default
                    clawutil_dir = os.path.expandvars("$CLAW/clawutil/src/")
                    makefile_src = os.path.join(clawutil_dir, "Makefile.common")
                shutil.copy(makefile_src, makefile_target)
                print(f"[DEBUG] Copied working Makefile to {rundir}")
                
            os.system('ln -sf ' + os.path.relpath(exe_source,rundir) + ' '+ os.path.join(rundir,'xgeoclaw'))
...
class GeoClawInput(object):
    ...
    # def write_files_01(self):
    #     """
    #     Write out all .data files for this GeoClaw run,
    #     including fgmax_grids.data, fgout_grids.data, and flagregions.data explicitly.
    #     """
    ...
    #     # core .data files to write
    #     core_files = [
    #             'amr.data','claw.data','dtopo.data','friction.data','gauges.data',
    #             'geoclaw.data','multilayer.data','qinit.data','refinement.data',
    #             'regions.data','surge.data','topo.data' ]

    #     # ---- 1) Preferred: write directly into run dir
    #     wrote_direct = False
    #     try:
    ...
    #     except TypeError:
    #         # older keyword? (just in case)
    #         try:
    #             self._rundata.write(outdir=rundir)  # not typical in >=5.8
    #             wrote_direct = True
    #             print(f"[DEBUG] Wrote standard .data files into {rundir} (outdir=)")
    #         except Exception as e:
    #             print("[DEBUG] Direct write failed, will fallback:", e)

    #     # ---- 2) Verify direct write; if any core file missing, fallback path
    #     missing = [f for f in core_files if not os.path.exists(os.path.join(rundir, f))]
    #     if wrote_direct and not missing:
    #         print("[DEBUG] Verified all core .data files present in run dir.")
    #     else:
    #         print(f"[DEBUG] Missing in run dir (will fallback): {missing}")

    #         # Fallback: write in CWD, then copy from CWD to run dir
    #         cwd = os.getcwd()
    #         try:
    #             self._rundata.write()
    #             print(f"[DEBUG] Wrote standard .data files into CWD: {cwd}")
    #         except Exception as e:
    #             print("[DEBUG] Fallback write in CWD failed:", e)
    #             # even if this fails, continue; explicit fgmax/fgout writes below
    ...
    #             else:
    #                 # final warning so you immediately see what's missing
    #                 print(f"[DEBUG][WARN] Still missing {fname}; expected at {src}")

    #     # ---- 3) Explicit fgout/fgmax/flagregions in run dir
    #     print("[DEBUG] FGout grids:", self._rundata.fgout_data.fgout_grids)
    #     print("[DEBUG] FGmax grids:", self._rundata.fgmax_data.fgmax_grids)

    #     try:
    #         self._rundata.fgout_data.write(os.path.join(rundir, 'fgout_grids.data'))
    #         print(f"[DEBUG] Wrote {os.path.join(rundir, 'fgout_grids.data')}")
    #     except Exception as e:
    #         print("[DEBUG] Skipping fgout write:", e)

    #     try:
    #         self._rundata.fgmax_data.write(os.path.join(rundir, 'fgmax_grids.data'))
    #         print(f"[DEBUG] Wrote {os.path.join(rundir, 'fgmax_grids.data')}")
    #     except Exception as e:
    #         print("[DEBUG] Skipping fgmax write:", e)

    #     try:
    #         self._rundata.flagregiondata.write(os.path.join(rundir, 'flagregions.data'))
    #         print(f"[DEBUG] Wrote {os.path.join(rundir, 'flagregions.data')}")
    #     except Exception as e:
    #         print("[DEBUG] Skipping flagregions write:", e)

    #     # ---- 4) Ensure xgeoclaw symlink
    #     common_exec = os.path.abspath(os.path.join(rundir, '..', '..', 'common', 'xgeoclaw'))
    #     run_exec = os.path.join(rundir, 'xgeoclaw')
    #     if not os.path.exists(common_exec):
    #         print(f"[DEBUG][WARN] xgeoclaw not found at {common_exec}. Build once with 'make new' in geoclaw_output/common/")
    #     elif not os.path.exists(run_exec):
    #         try:
    #             os.symlink(common_exec, run_exec)
    #             print(f"[DEBUG] Symlinked {run_exec} → {common_exec}")
    #         except Exception as e:
    #             print(f"[DEBUG] Could not symlink {run_exec}: {e}")
    #     else:
    #         print(f"[DEBUG] xgeoclaw already present in {rundir}")

    #     # Verify all expected .data files exist in the run directory
    #     data_fname_list = [
    #         'amr.data', 'claw.data', 'dtopo.data',
    #         'fgmax_grids.data', 'fgout_grids.data',
    #         'friction.data', 'gauges.data', 'geoclaw.data',
    #         'multilayer.data', 'qinit.data', 'refinement.data',
    #         'regions.data', 'surge.data', 'topo.data', 'flagregions.data'
    #     ]

    #     missing = []
    #     for fname in data_fname_list:
    #         fpath = os.path.join(rundir, fname)
    #         if not os.path.exists(fpath):
    #             missing.append(fname)

    #     if missing:
    #         print(f"[DEBUG] WARNING: Missing files in {rundir}: {missing}")
    #         # Try to copy from current driver directory if available
    #         for fname in missing:
    #             src = os.path.join(driver_home, fname)
    #             dest = os.path.join(rundir, fname)
    #             if os.path.exists(src):
    #                 shutil.copy(src, dest)
    #                 print(f"[DEBUG] Copied {src} → {dest}")
    #             else:
    #                 print(f"[DEBUG] Could not find {fname} in {driver_home}")
    #     else:
    #         print(f"[DEBUG] All expected .data files are present in {rundir}")
    ...
    def write_files(self, rundir=None):
        ...
        for fname in [
            ...
            if os.path.exists(fpath):
                ...
                print(f"[DEBUG] WARNING: {fname} not found in {rundir}")
        ...
        if not os.path.exists(exe):
            ...
            if os.path.exists(src_exe):
                ...
                print(f"[DEBUG] WARNING: xgeoclaw not found in {driver_home}")
        else:
            print(f"[DEBUG] xgeoclaw already present in {rundir}")

        # === Generate dtopo and plots ===
        dtopo_fname = os.path.join(rundir, "dtopo.tt3")
        dtopo_plot_fname = os.path.join(rundir, "dtopo_plot.png")
        subfaults_plot_fname = os.path.join(rundir, "subfaults_plot.png")
        ...
        # update dtopo filename in rundata
        if hasattr(self._rundata, "dtopo_data") and self._rundata.dtopo_data.dtopofiles:
            self._rundata.dtopo_data.dtopofiles[0][3] = dtopo_fname

        # create dtopography and write file
        ...
        fig0.savefig(subfaults_plot_fname, bbox_inches="tight")
        pl.close(fig0)
        print(f"[DEBUG] Saved subfaults plot → {subfaults_plot_fname}")

        # plot dtopo
        fig1, ax1 = pl.subplots()
        self.dtopo.plot_dZ_colors(0.0, axes=ax1)
        fig1.savefig(dtopo_plot_fname, bbox_inches="tight")
        pl.close(fig1)
        print(f"[DEBUG] Saved dtopo plot → {dtopo_plot_fname}")
